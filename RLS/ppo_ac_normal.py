...
from RLS.actors import Actor_GLU_NORMAL
from RLS.ac_util import initialize_weights , BatchMetrics, TrainingMetrics
from ppo_ac import PPO_AC


class PPO_AC_NORMAL(PPO_AC):

    def __init__(self, conf_space, state_dim, scenario):
        super().__init__(conf_space, state_dim, scenario)

        self.actor = Actor_GLU_NORMAL(state_dim, [len(p.bound) for p in self.cat_param], len(self.cont_param), scenario.num_blocks, scenario.num_neurons)
    ...
    def actions_to_conf(self, actions):
        ...
        for action in actions:
            ...
            for cont_action, param in zip(action_cont, self.cont_param):
                conf[param.name] = cont_action.item()
    ...
    def get_continuous_actions(self, continuous_mean, continuous_std):
        actions = []
        log_probs = []
        for mean, std, param in zip(continuous_mean, continuous_std, self.cont_param):

            parameter_range = param.bound
            dist = TanhNormal(mean.squeeze(1), std.squeeze(1), low=parameter_range[0], high=parameter_range[1], event_dims=0)
            action = dist.rsample()
    ...
    def calculate_log_probs_for_actions(self, state, actions_taken):
        ...
        logits, continuous_mean, continuous_std = self.actor(state)
        ...
        if len(self.cont_param) > 0:
            log_probs_cont = []
            entropies_cont = []
            for mean, std, action_taken, param in zip(continuous_mean, continuous_std, actions_taken_cont.transpose(0, 1), self.cont_param):
    ...
    def get_conf_prod(self, states):
        ...
        self.actor.eval()
        logits, continuous_mean, continuous_std = self.actor(states)
        ...
        if len(self.cont_param) > 0:
            ...
            for mean, std, param in zip(continuous_mean, continuous_std, self.cont_param):
                parameter_range = param.bound
                dist = TanhNormal(mean.squeeze(1), std.squeeze(1), low=parameter_range[0], high=parameter_range[1], event_dims=0)
                dist_mode = dist.get_mode() # mode or mean does not matter
                actions_cont.append(dist_mode)
